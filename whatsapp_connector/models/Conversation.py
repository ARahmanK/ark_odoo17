...
class AcruxChatConversation(models.Model):
    ...
    def create_contact(self, data):
        ...
        Stage_id = self.env['crm.stage'].sudo().search([('is_won', '=', True)])
        partner_id = Contact.search(['|', ('phone', '=', data['number']), ('mobile', '=', data['number'])])
        if not partner_id:
            partner_id = Contact.create({'name': data['name'] if not self.has_only_special_characters(data['name']) else partner_id.phone, 'phone': data['number'], 'category_id': [(4,1)]})
        if Lead.search([('partner_id', '=', partner_id.id), ('stage_id', '!=', Stage_id.id)]):
            pass
        else:
            self.create_oppertunity(data, partner_id)


    def create_oppertunity(self, data, partner_id):
        self.env['crm.lead'].sudo().create({
            'name': data['name'] if not self.has_only_special_characters(data['name']) else partner_id.phone,
            'type': 'opportunity',
            'partner_id': partner_id.id,
            'contact_name': partner_id.name,
            'phone': partner_id.phone,
            'tag_ids': [(4, self.env.ref('whatsapp_connector.whatsapp_to_crm_tag').id)]
        })

    def has_only_special_characters(self, input_string):
        special_characters = string.punctuation
        if all(char in special_characters for char in input_string):
            return True
        else:
            return False
